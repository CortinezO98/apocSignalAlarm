...
{
    [HttpPost("start")]
    public Task<Claim> Start([FromBody] Claim seed, CancellationToken ct) => service.StartAsync(seed, ct);

    [HttpPost("{id:guid}/otp")]
    public async Task<IActionResult> ValidateOtp(Guid id, [FromBody] string code, CancellationToken ct) =>
        await service.ValidateOtpAsync(id, code, ct) ? Ok() : BadRequest("OTP inválido");
